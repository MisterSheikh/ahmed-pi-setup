...
export const SUBAGENT_SPAWN_PROMPT_GUIDELINES = [
  ...
  "For subagent_spawn, pick the harness deliberately. Pi inherits the parent model and reasoning level when omitted. For bounded independent work where speed or cost matters, prefer opencode-go/deepseek-v4.1-flash; it supports high and max reasoning, with high as the normal choice.",
...
export const SUBAGENT_SPAWN_PARAMETER_DESCRIPTIONS = {
  ...
  model:
    'Model hint, interpreted by the chosen harness (pi: "provider/model-id" or model id; codex: model slug). Pi inherits the parent model when omitted; Codex defaults to gpt-5.6-sol.',
  reasoningEffort:
    "Reasoning effort on a shared scale; the harness maps it to its nearest native equivalent (pi thinking level, codex reasoning effort). Pi inherits the parent level when omitted; Codex defaults to high. DeepSeek V4.1 Flash permits high or max.",
};
