...
/** Claude has no `minimal` effort, so it maps to `low`. */
const CLAUDE_EFFORTS = {
  ...
  xhigh: "xhigh",
  max: "max",
} satisfies Record<ReasoningEffort, EffortLevel>;

export function claudeReasoningOptions(effort?: ReasoningEffort) {
  if (!effort) return {};
  return {
    effort: CLAUDE_EFFORTS[effort],
    thinking:
      effort === "off"
